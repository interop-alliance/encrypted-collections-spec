...
| wallet-core | `/keys` (the user-key wrap-set roster stores a `CollectionEncryption` descriptor verbatim), the ceremonies that rotate epochs (`/clients`, `/recovery`), and `src/resourceLog/` (the did:webvh controller adapter over vh-resource-log's controller port, and the ceremony-tail license supplied through its `admitAppend` hook). |
